...
export const KEYWORD_DESCRIPTIONS: Record<string, KeywordEntry> = {
  ...
  DEPLOY: {
    ...
    description: prose("유닛 혹은 베이스가 새롭게 배틀 에어리어에 놓였을 때 발동하는 효과."),
  },
  ATTACK: {
  ...
  PILOT: {
    ...
    description: prose("주로【세트 시】혹은【세트 중】뒤에 붙는, 효과 발동을 위한 파일럿 조건."),
  },
  WHEN_LINKED: {
    name: [{ type: "trigger", keyword: "WHEN_LINKED" }],
    description: prose("유닛의 링크 조건에 맞는 파일럿이 세트되었을 때 발동하는 효과."),
  },
  DURING_LINK: {
    name: [{ type: "trigger", keyword: "DURING_LINK" }],
    description: prose("유닛의 링크 조건에 맞는 파일럿이 세트되어 있는 동안 발동하는 효과."),
  },
  ONCE_PER_TURN: {
  ...
  FIRST_STRIKE: {
    ...
    description: prose("이 유닛이 공격한 배틀 중, 상대 유닛 혹은 베이스보다 먼저 대미지를 준다."),
  },
  HIGH_MANEUVER: {
    name: [{ type: "ability", keyword: "HIGH_MANEUVER" }],
    description: prose("이 유닛이 공격한 배틀 중, 상대 유닛의<블로커> 효과를 받지 않는다."),
  },
  SUPPRESSION: {
    name: [{ type: "ability", keyword: "SUPPRESSION" }],
    description: prose("공격으로 실드에 주는 대미지는, 선두부터 2개에 동시에 부여된다."),
  },
};
