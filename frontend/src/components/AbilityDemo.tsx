  ...
  return (
    ...
    <div
      ...
      style={{
        ...
        transition: "transform 350ms cubic-bezier(0.34,1.56,0.64,1), opacity 400ms, box-shadow 300ms",
      }}
    >
      <div className={cn("h-4 shrink-0", c.head)} />
      <div className="flex-1 flex items-center justify-center px-0.5">
        <span className={cn("text-[8px] font-bold text-center leading-tight break-keep", c.stat)}>
          {name}
        </span>
      </div>
      {tag && (
...
const DEMOS: Partial<Record<string, DemoConfig>> = {
  ...
  BLOCKER: {
    ...
    steps: [
      ...
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "→ 블로커로 대상 변경!", p2Label: "블로커 유닛 레스트 → 발동!",
        log: "블록 스텝: 블로커 유닛 레스트 → 공격 대상을 블로커로 변경" },
      { p1: d(), p2: d(), accent: null, p2Accent: "shield",
        p1Label: "배틀 종료", p2Label: "블로커가 대미지 수신 · 실드 무사!",
        log: "블로커가 배틀 대미지를 대신 받음 → 실드 에어리어 피해 없음" },
    ],
    p1Battle: (step) => (
      <MiniCard name="공격 유닛" ap={3} hp={3} color="red"
        rested={step >= 1} highlight={step === 1}
        attacking={step === 1 || step === 2} />
    ),
    p2Battle: (step) => (
      <div className="flex gap-1 items-center justify-center">
        <MiniCard name="블로커" ap={2} hp={step >= 3 ? 0 : 3} maxHp={3} color="blue"
          tag="블로커"
          dim={step === 0}
          rested={step >= 2}
          highlight={step === 2}
  ...
  FIRST_STRIKE: {
    ...
    steps: [
      { p1: d(), p2: d(), accent: null,
        p1Label: "나", p2Label: "상대", log: "" },
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "선제공격 → 먼저 대미지!", p2Label: "상대 유닛 대기",
        log: "선제공격: 상대 유닛보다 먼저 배틀 대미지를 준다" },
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "P2 유닛 파괴!", p2Label: "HP 0 → 파괴 → 반격 불가!",
        log: "P2 유닛 파괴 → 반격 단계 없음" },
      { p1: d(), p2: d(), accent: null,
        p1Label: "HP 그대로! (무손)", p2Label: "반격 없음",
        log: "선제공격으로 P1 유닛은 배틀 대미지를 받지 않음" },
    ],
    p1Battle: (step) => (
      <MiniCard name="선제 유닛" ap={3} hp={3} color="red"
        tag="선제공격"
        rested={step >= 1}
  ...
  HIGH_MANEUVER: {
    ...
    steps: [
      ...
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "블로커 무시!", p2Label: "블로커 효과 받지 않음",
        log: "고기동: 상대 유닛의 블로커 효과를 받지 않는다" },
      { p1: d(), p2: d({ shieldCount: 5 }), accent: null, p2Accent: "shield",
        p1Label: "플레이어 직접 대미지!", p2Label: "실드 1장 파괴!",
        log: "블로커가 있어도 실드 에어리어에 직접 대미지" },
    ],
    p1Battle: (step) => (
      <MiniCard name="고기동 유닛" ap={3} hp={3} color="red"
        tag="고기동"
        rested={step >= 1}
        highlight={step >= 2}
        attacking={step === 1 || step === 2}
    ...
    p2Battle: (step) => (
      <MiniCard name="블로커" ap={2} hp={3} color="blue"
        tag="블로커"
        dim={step === 0}
        highlight={step === 1}
      />
  ...
  SUPPORT: {
    ...
    p1Battle: (step) => (
      <div className="flex gap-1 items-center justify-center">
        <MiniCard name="원호 유닛" ap={1} hp={2} color="slate"
          tag="원호 2"
          dim={step === 0}
          rested={step >= 2}
          highlight={step === 1}
  ...
  REPAIR: {
    ...
    steps: [
      { p1: d(), p2: d(), accent: null,
        p1Label: "나 (HP 피해 상태)", p2Label: "상대", log: "" },
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "턴 종료 → 리페어 2 발동", p2Label: "상대",
        log: "리페어 X: 자신의 턴 종료 시 X만큼 HP를 회복한다" },
      { p1: d(), p2: d(), accent: "battle",
        p1Label: "HP +2 회복 완료!", p2Label: "상대",
        log: "유닛 HP 회복" },
    ],
    p1Battle: (step) => (
      <MiniCard name="리페어 유닛" ap={2} hp={step >= 2 ? 3 : 1} maxHp={3} color="green"
        tag="리페어 2"
        highlight={step >= 1}
      />
    ),
...
function DemoPlayer({ config, onPlayingChange }: {
  config: DemoConfig;
  onPlayingChange?: (playing: boolean) => void;
}) {
  const ref = useRef<HTMLDivElement>(null);
  const [step, replay] = useStepAnim(
    config.delays, ref,
...
export function AbilityDemo({ keyword, onPlayingChange }: {
  keyword: string;
  onPlayingChange?: (playing: boolean) => void;
}) {
