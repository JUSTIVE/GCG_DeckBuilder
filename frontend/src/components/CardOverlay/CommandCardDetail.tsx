...
export function CommandCardDetail({
  node,
  navigateWithFilter,
}: {
  node: any;
    ...
    <div className="pointer-events-auto max-h-[80dvh] w-72 overflow-y-auto rounded-xl bg-black/75 px-4 py-5 text-white backdrop-blur-md flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-white/60">
        ...
        <button
          ...
          onClick={() => navigateWithFilter({ package: node.package as any })}
        >
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs">
            <button
              ...
              onClick={() =>
                navigateWithFilter({
                  query: node.commandPilot!.name?.ko ?? node.commandPilot!.name,
                })
              }
            >
              {localize(node.commandPilot.name)}
            </button>
